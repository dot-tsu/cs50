...
bool check(char *word)
{
    // Initialize the cursor to the root of the trie
    node *cursor = root;

    // Iterate through each character in the input word
    for (int charIndex = 0; word[charIndex] != '\0'; charIndex++)
    {
        // Convert the current character to uppercase and calculate the corresponding array index
        int arrayIndex = toupper(word[charIndex]) - 'A';

        // Check if the current character's node is present in the trie
        if (cursor->children[arrayIndex] == NULL)
        {
            // If the node is not found, the word is not in the trie
            return false;
        }

        // Move the cursor to the next node corresponding to the current character
        cursor = cursor->children[arrayIndex];
    }

    // If the loop completes, the entire word exists in the trie
    return true;
}
